...
# your CPU (amd64 or arm64), so nothing is built at image time. It downloads the
# same docker compose stacks from this repo into an install directory (one source
# of truth) so you can manage them like the walkthrough in README.md.
#
...
# Download <url> -> <dest> with curl or wget. The compose stacks are fetched from
# the repo (REPO_RAW/REPO_REF) rather than embedded here, so there's a single
...
# Fetched from the repo so the compose file is a single source of truth. Image
# refs default to mehdishokohi/*:latest but are overridden by the .env below.
fetch "$REPO_RAW/$REPO_REF/platform/docker-compose.yml" "$INFLOW_DIR/platform/docker-compose.yml"
...
if [ "$INSTALL_INSPECTOR" = "1" ]; then
  ...
  # Fetched from the repo (single source of truth); image refs default to
  # mehdishokohi/*:latest but are overridden by the .env below.
  fetch "$REPO_RAW/$REPO_REF/inspector/docker-compose.yml" "$INFLOW_DIR/inspector/docker-compose.yml"
