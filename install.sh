...
#
# Stands up the platform (Infra + one Fractal) and, if you agree, the developer
# panel (inflow-inspector-api + inflow-inspector). It writes real docker compose
# stacks into an install directory so you can manage them afterward exactly like
# the manual walkthrough in README.md.
#
...
# ── optionally, the developer panel ───────────────────────────────────────────
if [ "$INSTALL_INSPECTOR" = "1" ]; then
  step "Writing developer panel stack -> $INFLOW_DIR/dev-panel"
  mkdir -p "$INFLOW_DIR/dev-panel"

  cat > "$INFLOW_DIR/dev-panel/docker-compose.yml" <<'YAML'
# Inflowenger developer panel — inflow-inspector-api + inflow-inspector.
# Joins the platform's external `inflow_net`. Generated by install.sh.
...
YAML
  ...
  step "Starting the developer panel"
  ( cd "$INFLOW_DIR/dev-panel" && $DC pull --quiet 2>/dev/null || true; $DC up -d )
  ok "panel started (http://localhost:8080)"
fi
...
[ "$INSTALL_INSPECTOR" = "1" ] && info "Stop panel             (cd $INFLOW_DIR/dev-panel && $DC down)"
printf '\n'
